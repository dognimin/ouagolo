...
if (isset($_GET['num']) && clean_data($_GET['num'])) {
    $parametres = array(
        ...
        'num_examen' => clean_data($_GET['num'])
    );
                            ...
                            if ($dossier_patient) {
                                ...
                                if ($dossier) {
                                    ...
                                    $age = DateTime::createFromFormat('Y-m-d', $dossier['date_naissance'], $tz)
                                        ...
                                        ->y;
                                    $bulletin = $DOSSIERS->trouver_bulletin_examens($dossier['code_dossier'], $parametres['num_examen']);
                                    if ($bulletin) {
                                        $actes = $DOSSIERS->lister_bulletins_examens_actes($bulletin['code']);
                                        $nb_actes = count($actes);
                                        if ($nb_actes != 0) {
                                            $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'IMPRESSION BULLETIN EXAMEN', json_encode($parametres));
                                            if ($audit['success'] == true) {
                                                ...
                                                $qr_code = array(
                                                    ...
                                                    'num_bulletin' => $bulletin['code'],
                                                    'date_bulletin' => date('d-m-Y', strtotime($bulletin['date_creation']))
                                                );
                                                ...
                                                $pdf->SetSubject('BULLETIN N° '.$bulletin['code']);
                                                ...
                                                $file_name = "BULLETIN_".$bulletin['code'].".pdf";
                                                ...
                                                $pdf->writeHTMLCell(100, 0, 50, 25, "BULLETIN D'EXAMENS", 1, 1, 0, true, 'C', true);
                                                ...
                                                $pdf->writeHTMLCell(0, 0, 10, 46, '<b>N° Bulletin:</b> '.$bulletin['code'], 0, 1, 0, true, 'L', true);
                                                ...
                                                $pdf->writeHTMLCell(0, 0, 10, 58, '<b>Date:</b> '.date('d/m/Y H:i', strtotime($bulletin['date_creation'])), 0, 1, 0, true, 'R', true);
                                                $pdf->SetFont('helvetica', 'B', 10, '', true);
                                                $pdf->SetFillColor(200, 220, 255);

                                                $pdf->Ln();
                                                $pdf->cell(30, 0, 'Code', 1, null, null, 1, null, null, null, null, null);
                                                $pdf->cell(158, 0, 'Nature', 1, 1, null, 1, null, null, null, null, null);

                                                $pdf->SetFont('helvetica', '', 10, '', true);
                                                $pdf->SetFillColor(255, 255, 255);
                                                foreach ($actes as $acte) {
                                                    $pdf->cell(30, 0, $acte['code'], 1, null, null, 1, null, null, null, null, null);
                                                    $pdf->cell(158, 0, $acte['libelle'], 1, 1, null, 1, null, null, null, null, null);

                                                }
                                                $pdf->SetFont('helvetica', 'B', 10, '', true);
                                                $pdf->SetFillColor(200, 220, 255);
                                                $pdf->cell(188, 0, 'Renseignements cliniques', 1, 1, 'C', 1, null, null, null, null, null);

                                                $pdf->SetFont('helvetica', '', 10, '', true);
                                                $pdf->SetFillColor(255, 255, 255);
                                                $pdf->cell(188, 80, $bulletin['renseignements'], 1, 1, null, 1, null, null, null, null, null);
