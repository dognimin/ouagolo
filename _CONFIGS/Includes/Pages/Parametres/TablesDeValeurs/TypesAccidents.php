...
<div id="div_datas">
    ...
    if($nb_typesaccidents == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune type accident n'a encore été enregistrée. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter une nouvelle</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($typesaccidents as $typesaccident) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($typesaccident['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $typesaccident['code'];?></td>
                    <td><?= $typesaccident['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($typesaccident['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $typesaccident['code'].'|'.$typesaccident['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $typesaccident['code'];?>|tac" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de devise monétaire');
