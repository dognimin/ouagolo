...
require_once "../../../../Classes/TYPESACCIDENTS.php";
$TYPESACCIDENTS = new TYPESACCIDENTS();
$types_accidents = $TYPESACCIDENTS->lister();
$nb_types_accidents = count($types_accidents);
?>
<p class="h4">Types Accidents</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_type_accident.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if ($nb_types_accidents == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun type accident n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    } else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            <?php
            $ligne = 1;
            foreach ($types_accidents as $type_accident) {
                $date_edition = date('Y-m-d', strtotime('+1 day', strtotime($type_accident['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $type_accident['code'];?></td>
                    <td><?= $type_accident['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($type_accident['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $type_accident['code'].'|'.$type_accident['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $type_accident['code'];?>|tac" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau type accident');
        return false;
    });
    ...
    $("#form_devise_monetaire").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_type_accident.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_devise_monetaire").hide();
                        $("#p_devise_monetaire_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('tac');
                        },5000);
                    }else {
                        $("#p_devise_monetaire_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de type accident');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=tac", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
