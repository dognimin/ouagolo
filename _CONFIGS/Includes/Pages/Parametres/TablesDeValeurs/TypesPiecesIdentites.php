...
$typespieces = $TYPESPIECES->lister();
$nb_typespieces = count($typespieces);
?>
<p class="h4">Types pièces d'indentité</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_types_pieces.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_typespieces == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun type de pièce d'identité n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm" id="tableDeValeurs">
            ...
            foreach ($typespieces as $typespiece) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($typespiece['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $typespiece['code'];?></td>
                    <td><?= $typespiece['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($typespiece['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $typespiece['code'].'|'.$typespiece['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $typespiece['code'];?>|tpi" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau type de pièce d\'identité');
        return false;
    });
    ...
    $("#form_types_pieces").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_types_pieces.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_types_pieces").hide();
                        $("#p_types_pieces_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('tpi');
                        },5000);
                    }else {
                        $("#p_types_pieces_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de type de pièce d\'identité');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=tpi", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
