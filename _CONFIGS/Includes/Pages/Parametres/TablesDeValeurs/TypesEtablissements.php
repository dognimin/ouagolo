...
?>
<p class="h4">Types établissements</p>
<p class="align_right">
...
<div id="div_datas">
    ...
    if($nb_types == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun type d'établissement n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($types as $type) {
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $type['code'];?>|tco" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $("#form_type_etablissement").submit(function () {
        ...
        if(libelle && code_niveau) {
            $("#button_enregistrer").prop('disabled', true)
                .removeClass('btn-primary')
                .addClass('btn-warning')
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_type_etablissement.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle,
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        ...
                        $("#p_type_etablissement_resultats").removeClass('alert alert-danger')
                            ...
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('tets');
                        },5000);
                    }else {
