...
$secteursactivites = $SECTEURSACTIVITES->lister();
$nb_secteuractivite = count($secteursactivites);
?>
<p class="h4">Secteurs d'Activites</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_secteur_activite.php";?>
                    </div>
...
<div id="div_datas">
    <?php
    if($nb_secteuractivite == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun secteur d'activité n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($secteursactivites as $secteursactivite) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($secteursactivite['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $secteursactivite['code'];?></td>
                    <td><?= $secteursactivite['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($secteursactivite['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $secteursactivite['code'].'|'.$secteursactivite['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $secteursactivite['code'];?>|sct" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau secteur d\'activité');
        return false;
    });
    ...
    $("#form_secteur_activite").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_secteur_activite.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_secteur_activite").hide();
                        $("#p_secteur_activite_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('sct');
                        },5000);
                    }else {
                        $("#p_secteur_activite_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de secteur d\'activité');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=sct", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
