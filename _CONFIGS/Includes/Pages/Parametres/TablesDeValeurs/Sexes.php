        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_sexe.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_sexes == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun sexe n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($sexes as $sexe) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($sexe['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $sexe['code'];?></td>
                    <td><?= $sexe['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($sexe['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $sexe['code'].'|'.$sexe['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $sexe['code'];?>|sex" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
    ...
    $("#form_sexe").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_sexe.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_sexe").hide();
                        $("#p_sexe_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('sex');
                        },5000);
                    }else {
                        $("#p_sexe_resultats").removeClass('alert alert-success')
