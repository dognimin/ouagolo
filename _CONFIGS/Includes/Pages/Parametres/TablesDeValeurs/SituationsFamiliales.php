...
$situationsfamiliales = $SITUATIONSFAMILIALES->lister();
$nb_situationsfamiliales = count($situationsfamiliales);
?>
<p class="h4">Situations Familiales</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_situation_familiale.php";?>
                    </div>
...
<div id="div_datas">
    <?php
    if($nb_situationsfamiliales == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune situation familiale n'a encore été enregistrée. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter une nouvelle</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            <?php
            $ligne = 1;
            foreach ($situationsfamiliales as $situationsfamiliale) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($situationsfamiliale['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $situationsfamiliale['code'];?></td>
                    <td><?= $situationsfamiliale['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($situationsfamiliale['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $situationsfamiliale['code'].'|'.$situationsfamiliale['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $situationsfamiliale['code'];?>|sif" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle situation familiale');
        return false;
    });
    ...
    $("#form_situation_familiale").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_situation_familiale.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_situation_familiale").hide();
                        $("#p_situation_familiale_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('sif');
                        },5000);
                    }else {
                        $("#p_situation_familiale_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de situation familiale');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=sif", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
