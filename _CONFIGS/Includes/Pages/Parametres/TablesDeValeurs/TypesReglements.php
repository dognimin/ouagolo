...
$nb_types_reglements = count($types_reglements);
?>
<p class="h4">Types règlements</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_type_reglement.php";?>
                    </div>
    ...
    if($nb_types_reglements == 0) {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($types_reglements as $type_reglement) {
                $date_edition = date('Y-m-d', strtotime('+1 day', strtotime($type_reglement['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $type_reglement['code'];?></td>
                    <td><?= $type_reglement['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($type_reglement['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $type_reglement['code'].'|'.$type_reglement['libelle'];?>" class="badge bg-<?php if ($validite_edition == 0) {echo 'secondary';} else {echo 'warning';}?> btn_edit" <?php if ($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $type_reglement['code'];?>|tac" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau type règlement');
        return false;
    });
    ...
    $("#form_type_reglement").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_type_reglement.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_type_reglement").hide();
                        $("#p_type_reglement_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('typ_reg');
                        },5000);
                    }else {
                        $("#p_type_reglement_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de type règlement');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=typ_reg", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
