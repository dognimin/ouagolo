...
require_once "../../../../Classes/CATEGORIESPROFESSIONNELSANTES.php";
$CPS = new CATEGORIESPROFESSIONNELSANTES();
$cps = $CPS->lister();
$nb_cps = count($cps);
?>
<p class="h4">Categories Professionnels Santé</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_categorie_professionnel_sante.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_cps == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune categorie n'a encore été enregistrée. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            <?php
            $ligne = 1;
            foreach ($cps as $categorie) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($categorie['date_debut'])));
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $categorie['code'];?>|cps" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle categorie');
        return false;
    });
    ...
    $("#form_categorie_professionnel_sante").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_categorie_professionnel_sante.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_categorie_professionnel_sante").hide();
                        $("#p_cps_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('cps');
                        },5000);
                    }else {
                        $("#p_cps_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de categorie proféssionnel santé');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=cps", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
