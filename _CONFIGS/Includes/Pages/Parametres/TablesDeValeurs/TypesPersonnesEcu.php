...
$typespersonnes = $TYPESPERSONNES->lister();
$nb_typespersonnes = count($typespersonnes);
?>
<p class="h4">Types personnes</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_type_personne_ecu.php";?>
                    </div>
...
<div id="div_datas">
    <?php
    if($nb_typespersonnes == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun type n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            <?php
            $ligne = 1;
            foreach ($typespersonnes as $typepersonne) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($typepersonne['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $typepersonne['code'];?></td>
                    <td><?= $typepersonne['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($typepersonne['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $typepersonne['code'].'|'.$typepersonne['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $typepersonne['code'];?>|typ_pers" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau type personne ECU');
        return false;
    });
    ...
    $("#form_type_ecu").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_type_ecu.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_type_ecu").hide();
                        $("#p_type_ecu_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('typ_pers');
                        },5000);
                    }else {
                        $("#p_type_ecu_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de type personne ECU');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=typ_pers", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
