...
?>
<p class="h4">Profils utilisateurs</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_profil_utilisateur.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_profils == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun profil n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($profils as $profil) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($profil['date_debut'])));
                ...
                <tr>
                    ...
                    <td class="align_center"><?= date('d/m/Y',strtotime($profil['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $profil['code'].'|'.$profil['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $profil['code'];?>|put" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau profil');
        return false;
    });
    ...
    $("#form_profil_utilisateur").submit(function () {
        ...
        if(libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_profil_utilisateur.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de profil');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=put", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
