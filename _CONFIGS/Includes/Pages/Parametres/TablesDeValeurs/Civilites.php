        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_civilites.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_civilites == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune civilité n'a encore été enregistrée. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter une nouvelle</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($civilites as $civilite) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($civilite['date_debut'])));
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" id="<?= $civilite['code'].'|'.$civilite['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $civilite['code'];?>|civ" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle civilité');
        return false;
    });
    ...
    $("#form_civilites").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_civilite.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_civilites").hide();
                        $("#p_civilite_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('civ');
                        },5000);
                    }else {
                        $("#p_civilite_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de civilité');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=civ", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
