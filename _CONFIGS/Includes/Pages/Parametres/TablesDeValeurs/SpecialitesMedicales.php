...
?>
<p class="h4">Spécialités médicales</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../_Forms/form_specialite_medicale.php";?>
                    </div>
...
<div id="div_datas">
    <?php
    if($nb_specialites_medicales == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune spécialité médicale n'a encore été enregistrée. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter une nouvelle</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm" id="tableDeValeurs">
            ...
            foreach ($specialites_medicales as $specialite_medicale) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($specialite_medicale['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $specialite_medicale['code'];?></td>
                    <td><?= $specialite_medicale['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($specialite_medicale['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $specialite_medicale['code'].'|'.$specialite_medicale['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $specialite_medicale['code'];?>|prf" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle spécialité médicale');
        return false;
    });
    ...
    $("#form_specialite_medicale").submit(function () {
        ...
        if(libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../_CONFIGS/Includes/Submits/Parametres/TablesDeValeurs/submit_specialite_medicale.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_specialite_medicale").hide();
                        $("#p_specialite_medicale_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_tables_de_valeurs('sme');
                        },5000);
                    }else {
                        $("#p_specialite_medicale_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de spécialités médicales');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("exporter-table-de-valeurs.php?type="+code_export+"&data=sme", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
