...
$groupes = $CLASSES_THERAPEUTHIQUES->lister_groupes();
$sous_groupes = $CLASSES_THERAPEUTHIQUES->lister_sous_groupes(null);
$nb_sous_groupes = count($sous_groupes);
?>
<p class="h4">Sous-groupes de médicament</p>
<p class="align_right">
    <button type="button" class="btn btn-primary btn-sm btn_add"><i class="bi bi-plus-square-fill"></i></button>
    <button type="button" id="med_group" class="btn btn-sm btn-secondary"><i class="bi bi-arrow-right-square"></i> Groupes</button>
</p>
<div id="div_form">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_medicament_sous_groupe.php";?>
                    </div>
...
<div id="div_datas">
    <?php
    if($nb_sous_groupes == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun sous-groupe n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            <tr>
                ...
                <th style="width: 100px">CODE GROUPE</th>
            ...
            foreach ($sous_groupes as $sous_groupe) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($sous_groupe['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $sous_groupe['code_groupe'];?></td>
                    <td><?= $sous_groupe['code'];?></td>
                    <td><?= $sous_groupe['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($sous_groupe['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $sous_groupe['code'].'|'.$sous_groupe['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $sous_groupe['code'];?>|med_cth" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau groupe');
        return false;
    });
    ...
    $("#form_sous_groupe").submit(function () {
        let code_groupe = $("#code_groupe_input").val().trim(),
            code        = $("#code_input").val().trim(),
            libelle     = $("#libelle_input").val().trim();
        if(code_groupe && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                .removeClass('btn-primary')
                .addClass('btn-warning')
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/Medicaments/submit_sous_groupe.php',
                type: 'POST',
                data: {
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_sous_groupe").hide();
                        $("#p_sous_groupe_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_medicaments('med_sgroup');
                        },5000);
                    }else {
                        $("#p_sous_groupe_resultats").removeClass('alert alert-success')
            ...
            if(!code_groupe) {
                $("#code_groupe_input").addClass('is-invalid');
                $("#codeGroupeHelp")
                    .addClass('text-danger')
                    .html("Veuillez sélectionner le code du groupe SVP.");
            }
            if(!code) {
    ...
    $("#med_group").click(function () {
        display_medicaments('med_group');
    });
