...
$presentations = $PRESENTATIONS->lister_presentations();
$nb_presentations = count($presentations);
?>
<p class="h4">Présentations / Conditionnements</p>
<p class="align_right">
...
<div id="div_datas">
    ...
    if($nb_presentations == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune présentation / conditionnement n'a encore été enregistrée. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter une nouvelle</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($presentations as $presentation) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($presentation['date_debut'])));
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" id="<?= $presentation['code'].'|'.$presentation['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $presentation['code'];?>|med_pre" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle Présentation');
        return false;
    });
    ...
    $("#form_presentation").submit(function () {
        ...
        if(libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/Medicaments/submit_presentation.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de Titre');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=med_pre", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
