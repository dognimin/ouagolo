...
?>
<p class="h4">Laboratoires pharmaceutiques</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_medicament_laboratoire_pharmaceutique.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_laboratoires == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun laboratoire n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($laboratoires as $laboratoire) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($laboratoire['date_debut'])));
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" id="<?= $laboratoire['code'].'|'.$laboratoire['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $laboratoire['code'];?>|med_lab" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau Laboratoire pharmaceutique');
        return false;
    });
    ...
    $("#form_laboratoire").submit(function () {
        ...
        if(libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/Medicaments/submit_medicaments_laboratoire_pharmaceutique.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_laboratoire").hide();
                        $("#p_laboratoire_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_medicaments('med_lab');
                        },5000);
                    }else {
                        $("#p_laboratoire_resultats").removeClass('alert alert-success')
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=med_lab", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
