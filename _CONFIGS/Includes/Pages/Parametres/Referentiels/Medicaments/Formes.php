...
?>
<p class="h4">Formes galéniques</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_medicament_forme.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_formes == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune forme n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($formes as $forme) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($forme['date_debut'])));
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" id="<?= $forme['code'].'|'.$forme['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $forme['code'];?>|med_frm" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle Forme');
        return false;
    });
    ...
    $("#form_forme").submit(function () {
        ...
        if(libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/Medicaments/submit_forme.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_forme").hide();
                        $("#p_forme_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_medicaments('med_frm');
                        },5000);
                    }else {
                        $("#p_forme_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de Forme');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=med_frm", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
