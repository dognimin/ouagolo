...
?>
<p class="h4">UNITES DE DOSAGES</p>
<p class="align_right">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_medicament_unite_dosage.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_unitesdosages == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune unite de dosage n'a encore été enregistrée. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($unitesdosages as $unitedosage) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($unitedosage['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $unitedosage['code'];?></td>
                    <td><?= $unitedosage['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($unitedosage['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $unitedosage['code'].'|'.$unitedosage['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $unitedosage['code'];?>|med_unt" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle unite de dosage');
        return false;
    });
    ...
    $("#form_classe_therapeuthique").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/Medicaments/submit_unite_dosage.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_classe_therapeuthique").hide();
                        $("#p_classe_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_medicaments('med_unt');
                        },5000);
                    }else {
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition d\'une unite de dosage  ');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=med_unt", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
