        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_pathologie_sous_chapitre.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_sous_chapitres == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun sous-chapitre n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        include "../../../_Forms/form_export.php";
        ?>
        <table class="table table-bordered table-hover table-sm table-striped" id="tableReferentiels">
            <thead class="bg-info">
            <tr>
                <th width="5">N°</th>
                <th width="10">CODE</th>
            ...
            foreach ($sous_chapitres as $sous_chapitre) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($sous_chapitre['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $sous_chapitre['code_chapitre'];?></td>
                    <td><?= $sous_chapitre['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($sous_chapitre['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $sous_chapitre['code'].'|'.$sous_chapitre['libelle'].'|'.$sous_chapitre['chapitre'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $sous_chapitre['code'];?>|pat_sch" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau sous-chapitre');
        return false;
    });
    $("#codeChapitresHelp").keyup(function () {
    ...
    $("#form_sous_chapitre").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/Pathologies/submit_pathologie_sous_chapitre.php',
                type: 'POST',
                data: {
                    'code': code,
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_sous_chapitre").hide();
                        $("#p_sous_chapitre_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_pathologies('pat_sch');
                        },5000);
                    }else {
                        $("#p_chapitre_resultats").removeClass('alert alert-success')
            ...
            if(!code_chapitre) {
                ...
                $("#codeChapitresHelp")
                    .addClass('text-danger')
                    .html("Veuillez sélectionner le chapitre SVP.");
            }
            if(!code) {
    ...
    $(".btn_edit").click(function () {
        ...
        let this_id = this.id,
            ...
            code = tableau[0],
            libelle = tableau[1],
            code_chapitre = tableau[2];
        $("#code_input").val(code).prop('disabled',true);
        $("#libelle_input").val(libelle);
        $("#code_chapitre_input").val(code_chapitre);


        $(".card-title").html('Edition de sous-chapitre');

    });

    $("#gsa").click(function () {
        display_tables_de_valeurs('souschap');
    });
    ...
    $('#tableReferentiels').DataTable();

    $("#export_input").change(function () {
        let code_export = $("#export_input").val();
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=pat_sch", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
