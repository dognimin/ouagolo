...
$PATHOLOGIESCHAPITRES = new PATHOLOGIES();
$chapitres = $PATHOLOGIESCHAPITRES->lister_chapitres();
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_pathologie_chapitre.php";?>
                    </div>
    ...
    if($nb_chapitres == 0) {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableReferentiels">
            ...
            foreach ($chapitres as $chapitre) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($chapitre['date_debut'])));
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" id="<?= $chapitre['code'].'|'.$chapitre['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $chapitre['code'];?>|pat_chap" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $("#form_chapitre").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/Pathologies/submit_pathologie_chapitre.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=pat_chap", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
