...
$actemedicaux = $ACTESMEDICAUX->lister_actes_medicaux(null);
...
$nb_actesmedicaux = count($actemedicaux);
?>
...
<div id="div_datas">
    ...
    if($nb_actesmedicaux == 0) {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            <?php
            $ligne = 1;
            foreach ($actemedicaux as $actemedical) {
                ...
                <tr>
                    ...
                    <td><?= $actemedical['titre_code'];?></td>
                    <td><?= $actemedical['chapitre_code'];?></td>
                    <td><?= $actemedical['section_code'];?></td>
                    <td><?= $actemedical['article_code'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($actemedical['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $actemedical['code'].'|'.$actemedical['libelle'].'|'.$actemedical['titre_code'].'|'.$actemedical['chapitre_code'].'|'.$actemedical['section_code'].'|'.$actemedical['section_code'].'|'.$actemedical['article_code'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $actemedical['code'];?>|act_med" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau Acte medical');
        return false;
    });
    ...
    $("#code_sections_input").change(function () {
        ...
        let code_titre     = $('#code_titre_input').val().trim(),
            chapitre_code     = $('#code_chapitres_input').val().trim(),
            section_code     = $(this).val().trim();
        if(section_code) {
