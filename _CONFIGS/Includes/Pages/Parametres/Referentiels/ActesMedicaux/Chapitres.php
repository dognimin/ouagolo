...
$nb_actesmedicaux = count($actesmedicaux);
?>
<p class="h4">Chapitres</p>
<p class="align_right">
        ...
        <div class="col-md-10">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_acte_medical_chapitre.php";?>
                    </div>
...
<div id="div_datas">
    <?php
    if($nb_actesmedicaux == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun chapitre n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($actesmedicaux as $actemedical) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($actemedical['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $actemedical['code_titre'];?></td>
                    <td><strong><?= $actemedical['code'];?></strong></td>
                    <td><?= $actemedical['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($actemedical['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $actemedical['code'].'|'.$actemedical['libelle'].'|'.$actemedical['code_titre'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $actemedical['code'];?>|act_cha" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouveau Chapitre');
        return false;
    });
    ...
    $("#code_titre_input").keyup(function () {
        let code     = $(this).val().trim();
        if(code) {
            ...
            $("#codeHelp")
                .addClass('text-danger')
                .html("Veuillez renseigner un Titre SVP.");
        }
    });
    ...
    $("#form_chapitre").submit(function () {
        let code    = $("#code_input").val().trim(),
            titre = $("#code_titre_input").val().trim(),
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/ActesMedicaux/submit_acte_medical_chapitre.php',
                type: 'POST',
                data: {
                    'code': code,
                    'titre': titre,
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_chapitre").hide();
                        $("#p_chapitre_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_actes_medicaux('act_cha');
                        },5000);
                    }else {
            ...
            if(!titre) {
                $("#code_titre_input").addClass('is-invalid');
                $("#codeTitreHelp")
                    .addClass('text-danger')
                    .html("Veuillez renseigner un Titre SVP.");
            }
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de chapitre');
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=act_cha", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
