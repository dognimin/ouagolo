        ...
        <div class="col-md-10">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_acte_medical_titre.php";?>
                    </div>
...
<div id="div_datas">
    ...
    if($nb_titres == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucun Titre n'a encore été enregistré. <br />Cliquez sur <a href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter un nouveau</p>
        <?php
    }else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($titres as $titre) {
                $date_edition = date('Y-m-d',strtotime('+1 day',strtotime($titre['date_debut'])));
                ...
                <tr>
                    ...
                    <td><?= $titre['code_titre'];?></td>
                    <td><?= $titre['libelle'];?></td>
                    <td class="align_center"><?= date('d/m/Y',strtotime($titre['date_debut']));?></td>
                    <td>
                        <button type="button" id="<?= $titre['code_titre'].'|'.$titre['libelle'];?>" class="badge bg-<?php if($validite_edition == 0) {echo 'secondary';}else {echo 'warning';}?> btn_edit" <?php if($validite_edition == 0) {echo 'disabled';} ?>><i class="bi bi-brush"></i></button>
                    </td>
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal" id="<?= $titre['code_titre'];?>|act_tit" data-bs-target="#historiqueModal"><i class="bi bi-clock-history"></i></button>
                    </td>
                </tr>
                <?php
                $ligne++;
...
<script>
    ...
    $("#form_titres").submit(function () {
        ...
        if(code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/ActesMedicaux/submit_acte_medical_titre.php',
                type: 'POST',
                data: {
                    'code': code,
                    'libelle': libelle
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_titres").hide();
                        $("#p_titre_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_actes_medicaux('act_tit');
                        },5000);
                    }else {
                        $("#p_titre_resultats").removeClass('alert alert-success')
    ...
    $("#export_input").change(function () {
        ...
        if(code_export) {
            window.open("../exporter-referentiels.php?type="+code_export+"&data=act_tit", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
