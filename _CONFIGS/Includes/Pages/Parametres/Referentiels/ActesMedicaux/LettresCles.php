        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-md-center">
                        <?php include "../../../_Forms/form_acte_medical_lettres_cles.php"; ?>
                    </div>
...
<div id="div_datas">
    ...
    if ($nb_lettres_cles == 0) {
        ?>
        <p class="align_center alert alert-warning">Aucune lettre clé n'a encore été enregistrée. <br/>Cliquez sur <a
                    href="" class="btn_add"><i class="bi bi-plus-square-fill"></i></a> pour ajouter une nouvelle</p>
        <?php
    } else {
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="tableDeValeurs">
            ...
            foreach ($lettres_cles as $lettre_cle) {
                $date_edition = date('Y-m-d', strtotime('+1 day', strtotime($lettre_cle['date_debut'])));
                ...
                <tr>
                    ...
                    <td class="align_right"><?= $lettre_cle['prix_unitaire']; ?> FCFA</td>
                    <td class="align_center"><?= date('d/m/Y', strtotime($lettre_cle['date_debut'])); ?></td>
                    <td>
                        <button type="button"
                                id="<?= $lettre_cle['code'] . '|' . $lettre_cle['libelle'] . '|' . $lettre_cle['prix_unitaire']; ?>"
                    ...
                    <td>
                        <button type="button" class="badge bg-dark button_historique" data-bs-toggle="modal"
                                id="<?= $lettre_cle['code']; ?>|let_cle" data-bs-target="#historiqueModal"><i
        ...
        <div class="modal fade" id="historiqueModal" tabindex="-1" aria-labelledby="historiqueModalLabel"
             aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="historiqueModalLabel"><i class="bi bi-clock-history"></i> Historique
                            des modifications</h5>
...
<script>
    ...
    $(".btn_add").click(function () {
        $("#div_datas").hide();
        $("#div_form").slideDown();
        $(".card-title").html('Nouvelle Lettre clé');
        return false;
    });
    ...
    $("#prix_input").keyup(function () {
        let prix = $(this).val().trim();
        if (jQuery.isNumeric(prix) === true) {
            if (prix) {
                $("#prix_input").removeClass('is-invalid')
                    .addClass('is-valid');
                $("#prixHelp")
                    .removeClass('text-danger')
                    .html("");
            } else {
                $("#prix_input").removeClass('is-valid')
                    .addClass('is-invalid');
                $("#prixHelp")
                    .addClass('text-danger')
    ...
    $("#form_lettres_cles").submit(function () {
        let code = $("#code_input").val().trim(),
            prix = $("#prix_input").val().trim(),
        ...
        if (code && libelle) {
            $("#button_enregistrer").prop('disabled', true)
                ...
                .html('<i>Traitement...</i>');
            $.ajax({
                url: '../../_CONFIGS/Includes/Submits/Parametres/Referentiels/ActesMedicaux/submit_acte_lettre_cle.php',
                type: 'POST',
                data: {
                    'code': code,
                    'prix': prix,
                ...
                success: function (data) {
                    ...
                    if (data['success'] === true) {
                        $("#form_lettres_cles").hide();
                        $("#p_lettres_resultats").removeClass('alert alert-danger')
                            .addClass('alert alert-success')
                            .html(data['message']);
                        setTimeout(function () {
                            display_actes_medicaux('let_cle');
                        }, 5000);
                    } else {
                        $("#p_lettres_resultats").removeClass('alert alert-success')
    ...
    $(".btn_edit").click(function () {
        ...
        $(".card-title").html('Edition de lettre clé');
    ...
    $("#export_input").change(function () {
        ...
        if (code_export) {
            window.open("../exporter-referentiels.php?type=" + code_export + "&data=let_cle", "PopupWindow", "width=600,height=600,scrollbars=yes,resizable=no");
        }
    });
</script>
