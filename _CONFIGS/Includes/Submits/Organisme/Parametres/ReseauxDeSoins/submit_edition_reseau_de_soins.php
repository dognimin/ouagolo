            ...
            if ($user) {
                ...
                if($user_profil) {
                    ...
                    if($organisme) {
                        ...
                        $edition = $RESEAUXDESOINS->editer($organisme['code'], $parametres['code'], strtoupper(conversion_caracteres_speciaux($parametres['libelle'])), $user['id_user']);
                        if ($edition['success'] == true) {
                            $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'EDITION ORGANISME RESEAU DE SOINS', json_encode($parametres));
