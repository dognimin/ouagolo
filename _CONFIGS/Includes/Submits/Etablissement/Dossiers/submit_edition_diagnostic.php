...
if (isset($_POST)) {
    ...
    $parametres = array(
        ...
        'diagnostic' => clean_data($_POST['diagnostic'])
    );
                    ...
                    if ($user_profil) {
                        ...
                        if ($ets) {
                            ...
                            if ($nb_modules !== 0) {
                                ...
                                if (in_array('AFF_DOSS', $modules, true) && in_array('EDT_DOS_DGTC', $sous_modules, true)) {
                                    $edition = $DOSSIERS->editer_diagnostic($parametres['code_dossier'], $parametres['diagnostic'], $user['id_user']);
                                    if ($edition['success'] == true) {
                                        $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'EDITION DIAGNOSTIC PATIENT', json_encode($parametres));
