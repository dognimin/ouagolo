...
if (isset($_POST)) {
    ...
    $parametres = array(
        ...
        'renseignements' => clean_data($_POST['renseignements'])
    );
                    ...
                    if ($user_profil) {
                        ...
                        if ($ets) {
                            ...
                            if ($nb_modules !== 0) {
                                ...
                                if (in_array('AFF_DOSS', $modules, true) && in_array('EDT_DOS_EXAMS', $sous_modules, true)) {
                                    $nb_actes = count($_POST['code']);
                                    $nb_actes_enregistres = 0;
                                    if ($nb_actes !== 0) {
                                        $edition = $DOSSIERS->editer_bulletin_examen($parametres['code_dossier'], $parametres['renseignements'], $user['id_user']);
                                        if ($edition['success'] === true) {
                                            for ($i = 0; $i < $nb_actes; $i++) {
                                                $acte = $DOSSIERS->editer_bulletin_examen_actes($edition['code'], $_POST['code'][$i], $user['id_user']);
                                                if ($acte['success'] === true) {
                                                    $nb_actes_enregistres++;
                                                }
                                            }
                                            if ($nb_actes === $nb_actes_enregistres) {
                                                $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'EDITION PATIENT BULLETIN EXAMEN', json_encode($parametres));
