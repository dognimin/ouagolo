...
if(isset($_POST)) {
    ...
    $parametres = array(
        ...
        'type_fin' => clean_data($_POST['type_fin'])
    );
                    ...
                    if ($user_profil) {
                        ...
                        if ($ets) {
                            ...
                            if ($nb_modules !== 0) {
                                ...
                                if (in_array('AFF_DOSS', $modules, true) && in_array('EDT_DOS', $sous_modules, true)) {
                                    $edition = $DOSSIERS->fermer_dossier($parametres['code_dossier'], $parametres['type_fin'], $parametres['date_fin'], $user['id_user']);
                                    if ($edition['success'] == true) {
                                        $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'EDITION FIN DOSSIER', json_encode($parametres));
