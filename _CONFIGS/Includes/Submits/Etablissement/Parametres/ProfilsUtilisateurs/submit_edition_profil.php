                        ...
                        if ($ets) {
                            ...
                            if ($nb_modules !== 0) {
                                ...
                                if (in_array('AFF_PRMTRS', $modules, true) && in_array('EDT_PRMTRS_PRFUT', $sous_modules, true)) {
                                    $edition = $ETABLISSEMENTS->editer_profil($ets['code'], $parametres['code'], strtoupper(conversion_caracteres_speciaux($parametres['libelle'])), $user['id_user']);
                                    if ($edition['success'] === true) {
                                        $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'EDITION ETS PROFIL UTILISATEUR', json_encode($parametres));
