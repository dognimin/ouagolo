                    ...
                    if ($user_profil) {
                        ...
                        if ($ets) {
                            ...
                            if ($nb_modules !== 0) {
                                ...
                                if (in_array('AFF_PRMTRS', $modules, true) && in_array('EDT_PRMTRS_CHBRS', $sous_modules, true)) {
                                    if ((int)$parametres['statut'] === 2) {
                                        $edition = $ETABLISSEMENTS->fermer_chambre($ets['code'], $parametres['code'], date('Y-m-d', time()), $user['id_user']);
                                    } else {
                                        $edition = $ETABLISSEMENTS->editer_chambre($ets['code'], $parametres['code'], strtoupper(conversion_caracteres_speciaux($parametres['libelle'])), $user['id_user']);
                                    }
                                    if ($edition['success'] === true) {
                                        $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'EDITION ETS CHAMBRE', json_encode($parametres));
