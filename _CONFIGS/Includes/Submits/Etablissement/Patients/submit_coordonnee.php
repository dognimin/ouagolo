...
if (isset($_POST)) {
    ...
    $parametres = array(
        'nip' => clean_data($_POST['num_patient']),
                    ...
                    if ($user_profil) {
                        ...
                        if ($ets) {
                            ...
                            if ($nb_modules !== 0) {
                                ...
                                if (in_array('AFF_PTS', $modules, true) && in_array('EDT_PT', $sous_modules, true)) {
                                    $edition = $POPULATIONS->editer_coordonnee($parametres['nip'], $parametres['type'], $parametres['valeur'], $user['id_user']);
                                    if ($edition['success'] === true) {
                                        $audit = $UTILISATEURS->editer_piste_audit($session['code_session'], ACTIVE_URL, 'EDITION COORDONNEE PATIENT', json_encode($parametres));
