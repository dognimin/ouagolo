    ...
    if(isset($_SESSION['nouvelle_session'])) {
        ...
        if ($session) {
            ...
            if($utilisateur) {
                ...
                $edition = $LOCALISATIONSGEOGRAPHIQUES->editer_commune(strtoupper(conversion_caracteres_speciaux($parametres['code_departement'])),strtoupper(conversion_caracteres_speciaux($parametres['code'])),strtoupper(conversion_caracteres_speciaux($parametres['nom'])),strtoupper(conversion_caracteres_speciaux($parametres['latitude'])),strtoupper(conversion_caracteres_speciaux($parametres['longitude'])),$utilisateur['id_user']);
                if($edition['success'] == true) {
                    $audit = $UTILISATEURS->editer_piste_audit($session['code_session'],ACTIVE_URL,'EDITION TABLE DE VALEURS COMMUNE',json_encode($parametres));
