                        ...
                        if ($ets) {
                            ...
                            if ($nb_modules !== 0) {
                                ...
                                if ($audit['success'] === true) {
                                    $organismes = $PATIENTS->lister_organismes($parametres['num_assure'], $parametres['date_soins']);
                                    $nb_organismes = count($organismes);
                                    if($nb_organismes != 0) {
                                        foreach ($organismes as $organisme) {
                                            $json[$organisme['code_organisme']] = $organisme['libelle_organisme'];
                                        }
                                    }else {
                                        $organismes = $ORGANISMES->lister();
                                        foreach ($organismes as $organisme) {
                                            $json[$organisme['code']] = $organisme['libelle'];
                                        }
                                    }
                                }else {
                                    $json = $audit;
