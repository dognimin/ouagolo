...
class ORDESNATIONNAUX extends BDD
{

    private function ajouter($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_ordres_nationnaux(orde_nationnal_code,orde_nationnal_libelle,orde_nationnal_date_debut, utilisateur_id_creation)
    ...
    private function fermer($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_ordres_nationnaux  SET orde_nationnal_date_fin = ?, date_edition = ?, utilisation_id_edition = ? WHERE orde_nationnal_code = ? AND orde_nationnal_date_fin IS NULL");
