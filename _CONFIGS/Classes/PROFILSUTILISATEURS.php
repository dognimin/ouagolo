...
class PROFILSUTILISATEURS extends  BDD
{
    private function ajouter($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_profils_utilisateurs(profil_utilisateur_code, profil_utilisateur_libelle, profil_utilisateur_date_debut, utilisateur_id_creation)
    ...
    private function fermer($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_profils_utilisateurs  SET profil_utilisateur_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE profil_utilisateur_code = ? AND profil_utilisateur_date_fin IS NULL");
