...
class TYPESPERSONNES extends BDD
{
    ...
    private function ajouter($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_types_personnes(type_personne_code,type_personne_libelle,type_personne_date_debut, utilisateur_id_creation)
    ...
    private function fermer($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_types_personnes  SET type_personne_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE type_personne_code = ? AND type_personne_date_fin IS NULL");
