...
class PATHOLOGIES extends BDD
{
    ...
    private function ajouter_chapitre($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_pathologies_chapitres(chapitre_code,chapitre_libelle,chapitre_date_debut,utilisateur_id_creation)
    ...
    private function ajouter_sous_chapitre($code_chapitre,$code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_pathologies_sous_chapitres(sous_chapitre_code,chapitre_code,sous_chapitre_libelle,sous_chapitre_date_debut,utilisateur_id_creation)
    ...
    private function ajouter_pathologie($code_sous_chapitre,$code,$libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_pathologies(pathologie_code,sous_chapitre_code,pathologie_libelle,pathologie_date_debut ,utilisateur_id_creation)
    ...
    private function fermer_chapitre($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_pathologies_chapitres  SET chapitre_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE chapitre_code = ? AND chapitre_date_fin IS NULL");
    ...
    private function fermer_sous_chapitre($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_pathologies_sous_chapitres  SET sous_chapitre_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE sous_chapitre_code = ? AND sous_chapitre_date_fin IS NULL");
        $a->execute(array($date_fin,date('Y-m-d H:i:s',time()),$user,$code));
        if ($a->errorCode() == "00000"){
            return array(
                "success" => true,
    ...
    private function fermer_pathologie($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_pathologies  SET 	pathologie_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE pathologie_code = ? AND pathologie_date_fin IS NULL");
        $a->execute(array($date_fin,date('Y-m-d H:i:s',time()),$user,$code));
        if ($a->errorCode() == "00000"){
            return array(
                "success" => true,
...
ORDER BY A.sous_chapitre_libelle
";
        $a = $this->bdd->prepare($query);
        $a->execute(array('%'.$code.'%'));
        return $a->fetchAll();
    }
    public function lister_pathologies($code) {
        $query = "
SELECT 
...
WHERE 
      A.pathologie_date_fin IS NULL AND B.sous_chapitre_date_fin IS NULL AND C.chapitre_date_fin IS NULL AND A.sous_chapitre_code LIKE ?
ORDER BY A.pathologie_libelle ASC
";
        $a = $this->bdd->prepare($query);
        $a->execute(array('%'.$code.'%'));
        return $a->fetchAll();
