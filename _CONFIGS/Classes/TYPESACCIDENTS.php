...
class TYPESACCIDENTS extends BDD
{
    private function ajouter($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_types_accidents(types_accidents_code,types_accidents_libelle,types_accidents_date_debut,utilisateur_id_creation)
    ...
    private function fermer($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_types_accidents  SET types_accidents_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE types_accidents_code = ? AND types_accidents_date_fin IS NULL");
