...
class LOCALISATIONSGEOGRAPHIQUES extends BDD
{
    private function ajouter_pays($code, $nom, $gentile, $indicatif_telephonique, $latitude, $longitude, $code_monnaie, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_geo_pays(pays_code,pays_nom,pays_gentile,pays_indicatif_telephonique,pays_latitude,pays_longitude,monnaie_code,pays_date_debut,utilisateur_id_creation)
    ...
    private function ajouter_region($code_pays, $code, $nom, $latitude, $longitude, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_geo_regions(pays_code,region_code,region_nom,region_latitude, region_longitude,region_date_debut,utilisateur_id_creation)
    ...
    private function ajouter_departement($code_region, $code, $nom, $latitude, $longitude, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_geo_departements(region_code,departement_code ,departement_nom,departement_latitude,departement_longitude,departement_date_debut ,utilisateur_id_creation)
    ...
    private function ajouter_commune($code_departement, $code, $nom, $latitude, $longitude, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_geo_communes(departement_code,commune_code,commune_nom,commune_latitude,commune_longitude,commune_date_debut ,utilisateur_id_creation)
    ...
    private function fermer_pays($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_geo_pays  SET pays_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE pays_code = ? AND pays_date_fin IS NULL");
    ...
    private function fermer_region($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_geo_regions SET region_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE region_code = ? AND region_date_fin IS NULL");
        $a->execute(array($date_fin,date('Y-m-d H:i:s',time()),$user,$code));
        if ($a->errorCode() == "00000"){
            return array(
                "success" => true,
    ...
    private function fermer_departement($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_geo_departements SET departement_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE departement_code = ? AND departement_date_fin IS NULL");
        $a->execute(array($date_fin,date('Y-m-d H:i:s',time()),$user,$code));
        if ($a->errorCode() == "00000"){
            return array(
                "success" => true,
    ...
    private function fermer_commune($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_geo_communes SET commune_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE commune_code = ? AND commune_date_fin IS NULL");
        $a->execute(array($date_fin,date('Y-m-d H:i:s',time()),$user,$code));
        if ($a->errorCode() == "00000"){
            return array(
                "success" => true,
...
SELECT 
       ...
       A.pays_gentile AS gentile,
       A.monnaie_code AS code_monnaie,
       B.monnaie_libelle AS libelle_monnaie,
       A.pays_indicatif_telephonique AS indicatif_telephonique,
       A.pays_drapeau_image AS drapeau,
       A.pays_latitude AS latitude,
       A.pays_longitude AS longitude,
...
FROM 
     tb_ref_geo_pays A 
         JOIN tb_ref_monnaies B 
             ON 
                 A.monnaie_code = B.monnaie_code AND 
      A.pays_date_fin IS NULL
ORDER BY A.pays_nom
";
        $a = $this->bdd->prepare($query);
...
FROM
     tb_ref_geo_regions A 
         JOIN tb_ref_geo_pays B 
             ON A.pays_code = B.pays_code AND
      A.region_date_fin IS NULL AND A.pays_code LIKE ?
...
FROM
     tb_ref_geo_communes A 
         ...
         JOIN tb_ref_geo_pays D 
             ON C.pays_code = D.pays_code AND
                A.commune_date_fin IS NULL AND 
                A.departement_code LIKE ?
ORDER BY A.departement_code, A.commune_nom
        ";
        $a = $this->bdd->prepare($query);
    ...
    public function editer_pays($code, $nom, $gentile, $indicatif_telephonique, $latitude, $longitude, $code_monnaie, $user){
        $civilite = $this->trouver_pays($code);
        if($civilite) {
            $date_fin = date('Y-m-d',strtotime('-1 day',time()));
            if(strtotime($date_fin) > strtotime($civilite['date_debut'])) {
                $edition = $this->fermer_pays($civilite['code'],$date_fin,$user);
    ...
    public function editer_region($code_pays, $code, $nom, $latitude, $longitude, $user){
        $civilite = $this->trouver_region($code);
        if($civilite) {
            $date_fin = date('Y-m-d',strtotime('-1 day',time()));
            if(strtotime($date_fin) > strtotime($civilite['date_debut'])) {
                $edition = $this->fermer_region($civilite['code'],$date_fin,$user);
                if($edition['success'] == true) {
                    $json = $this->ajouter_region($code_pays, $code, $nom, $latitude, $longitude, $user);
                }else {
                    $json = $edition;
                }
            }else {
    ...
    public function editer_departement($code_region, $code, $nom, $latitude, $longitude, $user){
        ...
        if($departement) {
            ...
            if(strtotime($date_fin) > strtotime($departement['date_debut'])) {
                ...
                if($edition['success'] == true) {
                    $json = $this->ajouter_departement($code_region, $code, $nom, $latitude, $longitude, $user);
                }else {
                    $json = $edition;
                }
            }else {
    ...
    public function editer_commune($code_departement, $code, $nom, $latitude, $longitude, $user){
        ...
        if($commune) {
            ...
            if(strtotime($date_fin) > strtotime($commune['date_debut'])) {
                ...
                if($edition['success'] == true) {
                    $json = $this->ajouter_commune($code_departement, $code, $nom, $latitude, $longitude, $user);
                }else {
                    $json = $edition;
                }
            }else {
