...
class SECTEURSACTIVITES extends BDD
{
    private function ajouter($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_secteurs_activites(secteur_activite_code,secteur_activite_libelle,secteur_activite_date_debut,utilisateur_id_creation)
    ...
    private function fermer($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_secteurs_activites  SET secteur_activite_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE secteur_activite_code = ? AND secteur_activite_date_fin IS NULL");
