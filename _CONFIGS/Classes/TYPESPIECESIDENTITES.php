...
class TYPESPIECESIDENTITES extends  BDD
{
    private function ajouter($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_types_pieces_identites(type_piece_identite_code,type_piece_identite_libelle,type_piece_identite_date_debut, utilisateur_id_creation)
    ...
    private function fermer($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_types_pieces_identites  SET type_piece_identite_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE type_piece_identite_code = ? AND type_piece_identite_date_fin IS NULL");
