...
class DEVISESMONETAIRES extends  BDD
{
    private function ajouter($code, $libelle, $user){
        $a = $this->bdd->prepare("INSERT INTO tb_ref_monnaies(monnaie_code,monnaie_libelle,monnaie_date_debut, utilisateur_id_creation)
    ...
    private function fermer($code, $date_fin, $user) {
        $a = $this->bdd->prepare("UPDATE tb_ref_monnaies SET monnaie_date_fin = ?, date_edition = ?, utilisateur_id_edition = ? WHERE monnaie_code = ? AND monnaie_date_fin IS NULL");
