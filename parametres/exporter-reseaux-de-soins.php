...
if($type == 'csv') {
    if($data == 'res_etab') {
        ...
        if($nb_reseaux_etablissements != 0) {
            ...
            header('Content-Disposition: attachment; filename="EXPORT_RESEAUX_DE_SOINS_ETABLISSEMENTS_' . date('dmYhis', time()) . '.csv"');
    ...
    elseif($data == 'res') {
        ...
        if($nb_reseaux != 0) {
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="EXPORT_RESEAUX_DE_SOINS_' . date('dmYhis', time()) . '.csv"');
            //$donnees = array('CODE', 'LIBELLE', 'DATE EFFET');
    ...
    elseif($data == 'res_acte') {
        ...
        if($nb_reseaux_actes != 0) {
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="EXPORT_RESEAUX_DE_SOINS_ACTES_' . date('dmYhis', time()) . '.csv"');
            //$donnees = array('CODE', 'LIBELLE', 'DATE EFFET');
...
elseif ($type == 'pdf') {
    ...
    if ($data == 'res_acte') {
        ...
        if($nb_actes != 0) {
            ...
            $pdf->MultiCell(150, 5, 'CODE ACTE', 1, 'L', 1, 0, '', '', true);
            ...
            foreach ($reseaux_actes as $reseau_acte) {
                ...
                $pdf->MultiCell(100, 5, $reseau_acte['code_reseau'], 1, 'L', 1, 0, '', '', true);
                $pdf->MultiCell(150, 5, $reseau_acte['code_acte'], 1, 'L', 1, 0, '', '', true);
    ...
    elseif ($data == 'res') {
        ...
        if($nb_reseaux != 0) {
            ...
            $pdf->writeHTMLCell(0, 0, 50, 15, '<h1>RESEAUX DE SOINS:<br />RESEAUX DE SOINS</h1>', 0, 1, 0, true, 'C', true);
            $pdf->setCellPaddings(1, 1, 1, 1);
            $pdf->MultiCell(10, 15, NULL, 0, 'L', 1, 1, '', '', true);
            ...
            foreach ($reseaux as $reseau) {
                $pdf->SetFont('helvetica', 'B', 8, '', true);
                $pdf->MultiCell(10, 5, $ligne, 1, 'R', 1, 0, '', '', true);
                $pdf->MultiCell(100, 5, $reseau['code'], 1, 'L', 1, 0, '', '', true);
                $pdf->MultiCell(150, 5, $reseau['libelle'], 1, 'L', 1, 0, '', '', true);
